...
application {
    mainModule.set("test.aggregate.handle.occurrence.service")
    mainClass.set("io.github.big.andy.coates.test.aggregate.handle.occurrence.service.ServiceMain")
}
...
val buildAppImage = tasks.register<DockerBuildImage>("buildAppImage") {
    ...
    images.add("ghcr.io/big-andy-coates/${rootProject.name}-${project.name}:latest")
    images.add("ghcr.io/big-andy-coates/${rootProject.name}-${project.name}:${project.version}")
}
